...
## 2026-04-14 — 툴별 Instruction 파일 위치와 명명 규칙
- **Claude CLI**: `.claude/CLAUDE.md` 또는 `.claude/instructions.md` — 프로젝트 루트 기준으로 탐색
- **Gemini CLI**: `.gemini/GEMINI.md` — 프로젝트 루트에 위치
- **VS Code Copilot**: `.vscode/settings.json`의 `github.copilot.chat.codeGeneration.instructions` 배열에 파일 경로 등록 필요
- 툴마다 읽는 파일 이름이 다르므로, `make skills-sync` 같은 자동화 스크립트로 Single Source of Truth(`.github/skills/`)에서 각 툴 경로로 배포하는 패턴이 필수
